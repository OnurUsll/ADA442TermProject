...
# --- If you used any custom class/function in the pipeline, define/import it here ---
# from my_custom_module import CustomTransformer  <-- if it's in another file
...
try:
    ...
    st.error("❌ Failed to load the model. Make sure 'full_pipeline.pkl' exists and any custom transformers are defined.")
    st.exception(e)
    st.stop()
...
with center:
    ...
    if st.button("Predict"):
        ...
        try:
            ...
            st.error("Prediction failed. Please check your model or input formatting.")
            st.exception(e)
            st.stop()
